...
class _SocialInvolvementScreenState extends State<SocialInvolvementScreen> {
  ...
  Future<void> _loadSocialDetails() async {
    ...
    final data = doc.data() ?? {};
    final rawSocials = Map<String, dynamic>.from(data['socialInvolvement'] ?? {});
  ...
  Future<void> _saveSocialDetails() async {
    ...
    await FirebaseFirestore.instance.collection('users').doc(id).set({
      'socialInvolvement': _socialMapFromFields(),
  ...
  Widget build(BuildContext context) {
    ...
    final background = isDark ? AppColors.darkBackground : const Color(0xFFF7F5FF);

    return Scaffold(
      backgroundColor: background,
      appBar: AppBar(
        title: Text(widget.sharingMode ? 'Share Contact Methods' : 'Connect Beyond NIMA'),
      ),
      body: SafeArea(
        child: loading
            ...
            : SingleChildScrollView(
                ...
                child: Column(
                  ...
                  children: [
                    Text(
                      widget.sharingMode ? 'Share with this Pulse' : 'Connect Beyond NIMA',
                      style: TextStyle(
                        color: textColor,
                        fontSize: 31,
                        height: 1.08,
                    ...
                    Row(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        const Icon(Icons.lock_rounded, color: AppColors.royalPurple, size: 19),
                    ...
                    if (widget.sharingMode)
                      _SharingContent(
                        ...
                        onChanged: (key, value) {
                          setState(() {
                            value ? selectedKeys.add(key) : selectedKeys.remove(key);
                          });
                        },
                      )
                    else
                    ...
                    SizedBox(
                      ...
                      child: ElevatedButton.icon(
                        onPressed: saving ? null : widget.sharingMode ? _shareSelected : _saveSocialDetails,
                        icon: saving
                            ? const SizedBox(
                                width: 18,
                                height: 18,
                            ...
                            : Icon(widget.sharingMode ? Icons.ios_share_rounded : Icons.check_circle_rounded),
                        label: Text(
                          widget.sharingMode ? 'Share Selected' : saving ? 'Saving...' : 'Create Profile',
...
class _SharingContent extends StatelessWidget {
  ...
  final String Function(String key) iconName;
    ...
    if (socials.isEmpty) {
      return Container(
        ...
        child: Text(
          ...
          style: TextStyle(color: mutedColor, height: 1.4, fontWeight: FontWeight.w600),
        ),
      );
    }
      ...
      children: socials.entries.map((entry) {
        ...
        return Container(
          ...
          child: CheckboxListTile(
            ...
            secondary: NimaSvgIcon(name: iconName(key), size: 23),
            title: Text(displayName(key), style: const TextStyle(fontWeight: FontWeight.w900)),
            subtitle: Text(value),
          ),
        );
      }).toList(),
...
class _EditingContent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        _SocialField(controller: telegramController, label: 'Telegram', hint: 'Username', iconName: 'telegram'),
        _SocialField(controller: instagramController, label: 'Instagram', hint: 'Username', iconName: 'instagram'),
        _SocialField(controller: facebookController, label: 'Facebook', hint: 'Profile name or username', iconName: 'facebook'),
        _SocialField(controller: snapchatController, label: 'Snapchat', hint: 'Username', iconName: 'snapchat'),
        _SocialField(controller: tiktokController, label: 'TikTok', hint: 'Username', iconName: 'tiktok'),
        _SocialField(controller: xController, label: 'X (Twitter)', hint: 'Username', iconName: 'x'),
        _SocialField(controller: linkedinController, label: 'LinkedIn', hint: 'Public profile username', iconName: 'linkedin'),
        _SocialField(
      ...
      child: TextField(
        ...
        decoration: InputDecoration(
          ...
          prefixIcon: Padding(
            ...
            child: NimaSvgIcon(name: iconName, size: 21),
          ),
          filled: true,
          ...
          focusedBorder: const OutlineInputBorder(
            ...
            borderSide: BorderSide(color: AppColors.royalPurple, width: 1.4),
          ),
        ),
      ),
    );
